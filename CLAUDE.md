...
- **LlamaStack playground — RAG requires vLLM tool-calling flags**: When RAG (Knowledge) is toggled on in the Playground, LlamaStack's Responses API sends `tool_choice: "auto"` to vLLM so the model can invoke the `file_search` tool for retrieval. Without `--enable-auto-tool-choice` and `--tool-call-parser hermes` in the `LLMInferenceService` args, vLLM returns HTTP 400 (`"auto" tool choice requires --enable-auto-tool-choice and --tool-call-parser to be set`) and the Playground shows an empty response bubble. These flags are included in `manifests/07/llm-inference-qwen25-3b-instruct.yaml` and `manifests/07/llm-inference-qwen25-7b-instruct.yaml`. `hermes` is the correct parser for Qwen2.5 models (Hermes 2 Pro function-calling format).
