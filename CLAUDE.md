...
- **LlamaStack playground — RAG (Knowledge) is ready out of the box**: The `rh-dev` distribution used by the auto-generated `LlamaStackDistribution` already includes all RAG components: `inline::milvus` (vector IO), `inline::sentence-transformers` with IBM Granite `granite-embedding-125m-english` (768-dim embeddings), `inline::file-search` (tool runtime), and `inline::localfs` (file storage). The `vector_io` API and `vector_stores.default_provider_id: milvus` are pre-configured in the `llama-stack-config` ConfigMap. No additional setup is needed — toggle RAG on in the Playground Knowledge tab and upload files. Files and embeddings are stored on ephemeral pod storage and lost on pod restart (no PVC configured).
