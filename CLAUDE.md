...
# The Kuadrant Operator does NOT auto-create a Limitador CR; create it manually so the Limitador
# pod starts and rate limiting infrastructure is ready (required for MaaSSubscription tokenRateLimits).
...
- **MaaSSubscription `Degraded` — RHCL 1.4 Limitador Operator CRD bug**: If `MaaSSubscription` shows `Degraded` with `[Limitador Operator] is not installed`, this is a known RHCL 1.4 bug. The `MaaSSubscription` `tokenRateLimits` field (required by the CRD schema — cannot be omitted) creates a `TokenRateLimitPolicy` that depends on Limitador. The Kuadrant Operator (rhcl-operator.v1.4.0) checks for the `limitadoroperators.limitador.kuadrant.io` CRD to confirm Limitador Operator readiness, but this CRD was removed in Limitador Operator 1.x (only `limitadors.limitador.kuadrant.io` exists). Even with a running Limitador pod (`oc apply -f manifests/02/limitador-instance.yaml` starts it), the `KuadrantStatusUpdater` in the Kuadrant Operator still reports `MissingDependency` because the old CRD name no longer exists. Impact: the `TokenRateLimitPolicy` is not accepted and rate limits are not enforced by Limitador, but authentication, authorization (`MaaSAuthPolicy Active`), and chat completions all work correctly. `MaaSAuthPolicy` remaining `Active` is the operationally critical resource.
...
- **MaaS authentication model — API key vs OpenShift token**: The MaaS gateway (`maas.<apps-domain>`) accepts two Bearer token types: (1) an OpenShift identity token (`oc whoami -t`) — valid for any user in `system:authenticated`, including `kubeadmin`; (2) a MaaS API key generated in Gen AI Studio → API keys. The **Playground** bypasses the MaaS gateway entirely — it connects directly to the LlamaStack distribution inside the cluster using the user's RHOAI dashboard session (OpenShift OAuth). No API key is ever needed for the Playground. API keys are only necessary for callers that have no OpenShift account: external apps, CI pipelines, or teammates sharing model access. OpenShift tokens expire (~24 h); MaaS API keys are long-lived (1–365 days) and better suited for persistent integrations.
