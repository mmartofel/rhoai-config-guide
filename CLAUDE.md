...
- **LlamaStack playground — RAG requires vLLM tool-calling flags AND wider context window**: Two `LLMInferenceService` args are required for RAG to work. (1) `--enable-auto-tool-choice` + `--tool-call-parser hermes`: LlamaStack sends `tool_choice: "auto"` when invoking `file_search`; without these flags vLLM returns HTTP 400. `hermes` is correct for Qwen2.5 (Hermes 2 Pro function-calling format). (2) `--max-model-len=8192`: RAG injects retrieved chunks into the prompt, easily pushing input to 3000+ tokens. With `max_model_len=4096` and `VLLM_MAX_TOKENS=1024`, there are only 3072 tokens of input budget — a single large chunk tips it over ("total of at least 4097 tokens" error). 8192 gives 7168 tokens of input budget. Safe for T4 + Qwen2.5-3B: KV cache for 8192 tokens ≈ 1.1 GiB (available after weights: ~6.75 GiB). Both flags are in `manifests/07/llm-inference-qwen25-3b-instruct.yaml`. The 7B manifest keeps `max-model-len=4096` — the 7B model exhausts T4 VRAM; RAG on 7B requires ≥20 GiB VRAM.
